...
def lotting(
    ...
    energy = np.sum(
        [1 - Lambda * L.metrics(suppliersIndicators) * len(L) / clusterSize for L in Ls]
    )
    ...
    for i in range(iterationsAmount):
        ...
        if conjunction and (length >= 2):
            ...
            if lotTimeViable(L1, L2, timeConstraint):
                ...
                e1 = (
                    1 - Lambda * L1.metrics(suppliersIndicators) * len(L1) / clusterSize
                )
                e2 = (
                    1 - Lambda * L2.metrics(suppliersIndicators) * len(L2) / clusterSize
                )
                e3 = (
                    1 - Lambda * L3.metrics(suppliersIndicators) * len(L3) / clusterSize
                )
                deltaE = e3 - e2 - e1
            ...
            if divideByClass:
                ...
                dividerIndices = np.random.choice(
                    np.nonzero(Ls[k].classesVector())[0],
                    size=dividerSize,
                    replace=False,
                )
                ...
                e0 = (
                    1
                    - Lambda
                    * Ls[k].metrics(suppliersIndicators)
                    * len(Ls[k])
                    / clusterSize
                )
